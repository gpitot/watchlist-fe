...
export const MovieModal: React.FC<{
  ...
  isOpen: boolean;
  onModalClose: () => void;
}> = ({ movie, trendingItem, isOpen, onModalClose }) => {
  ...
  const { mutateAsync: mutateAsyncAddMovie } = useAddMovie();

  // Determine which type of item we're showing
  const isTrendingItem = !movie && !!trendingItem;
  ...
  const addTrendingToList = async () => {
    if (!trendingItem) {
      return;
    }
    await mutateAsyncAddMovie({
      id: trendingItem.movie_id,
      medium: trendingItem.medium,
      streamData: trendingItem,
    });
    onModalClose();
  };

  if (!movie && !trendingItem) {
    return null;
  }

  // Get data from either movie or trending item
  const title = movie?.title ?? trendingItem?.name ?? "";
  const description = movie?.description ?? trendingItem?.description ?? "";
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 sm:space-y-5">
          ...
          {!isSharing && (
            <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3 pt-3 sm:pt-4 border-t border-border-default">
              {isTrendingItem ? (
                <button
                  onClick={addTrendingToList}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3 sm:py-2.5 rounded-xl bg-primary/10 active:bg-primary/20 sm:hover:bg-primary/20 text-primary-light active:text-primary-lighter sm:hover:text-primary-lighter border border-primary/20 active:border-primary/30 sm:hover:border-primary/30 transition-colors text-sm font-medium active:scale-[0.98]"
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
